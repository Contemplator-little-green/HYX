...
class Curvphormer(nn.Module):
    def __init__(self,
        ...
        self.attn_readout = GlobalAttention(
    ...
    def compute_dynamic_curvatures(self, h, edge_index, layer_idx, epoch_ratio=0.0):
        src, tgt = edge_index[0], edge_index[1]
        h_src, h_tgt = h[src], h[tgt]
        sim = (h_src * h_tgt).sum(dim=-1)
        # ------------------- 修改开始: Beta 退火改进 -------------------
        # 假设 Curvphormer.forward 中传入 epoch_ratio（训练进度，0~1）
        # 在类初始化时新增 self.global_beta_factor = 1.0  # 可调
        current_beta = self.beta * (layer_idx + 1) / self.num_layers
        # 训练进度加成，使 beta 随训练时间增加
        current_beta = current_beta * (1.0 + self.global_beta_factor * epoch_ratio)
        # ------------------- 修改结束 -------------------
    ...
    def forward(self, data, epoch_ratio=0.0):
        ...
        for i, layer in enumerate(self.layers):
            if self.use_dynamic_curvature:
                curvature_embeddings = self.compute_dynamic_curvatures(h, edge_index, i, epoch_ratio)
            if edge_emb is not None:
                curvature_embeddings = self.fusion_mlp(curvature_embeddings, edge_emb)
            h = layer(h, edge_index, curvature_embeddings)

        graph_out = self.attn_readout(h, batch)
        return self.output_layer(graph_out)

# ------------------- 训练/评估函数保持原样 -------------------
def train_epoch(model, loader, optimizer, criterion, device, epoch_ratio=0.0):
    model.train()
    total_loss = 0
    ...
    for data in pbar:
        ...
        out = model(data, epoch_ratio=epoch_ratio).squeeze()
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(1, num_epochs + 1), desc="Epochs"):
        epoch_ratio = (epoch - 1) / (num_epochs - 1)  # 从 0~1

        train_mse, train_mae, train_rmse, train_r2 = train_epoch(model, train_loader, optimizer, criterion, device, epoch_ratio)
